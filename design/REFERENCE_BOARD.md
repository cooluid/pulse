...
- 主参考：用户提供的蓝色湿度状态卡（2026-08-21）；辅助参考：[Your black horizon](https://olafureliasson.net/artwork/your-black-horizon-2005/)、[Teenage Engineering Designs](https://teenage.engineering/designs)。
- 借：深蓝到亮蓝的全幅空间、左侧强弱信息层级、右侧细轴与白色节点、底部多层色面；黑场里单一边界建立空间；物理控件用位置而非装饰表达状态。
- 落点：Watch 玻璃本身就是蓝色状态场，不再内套卡片。左侧显示「今日」与当前动作/状态，右侧日轴只表达待签到、确认中和已确认的离散位置；底部波面在前台高亮时错速缓慢漂移，受 Reduce Motion / Always-On 门控，并为每个正式状态保留可读的静态终态。
- 不借：湿度数值、水滴、露点文案、可拖动滑杆、连续进度、蓝紫玻璃卡片墙、霓虹科幻光效、硬件玩具质感，以及任何没有领域事实支持的统计。
...
| Apple Watch | R5 | 全幅蓝色状态场；左侧动作与事实，右侧二态日轴，右上留给系统时间 |
