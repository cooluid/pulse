...
| App Store 动作 | 在 App Store 评价 | Rate Pulse on the App Store | 五星好评、支持我们、喜欢就评分 |
...
3. Widget 画廊文案只需准确对应当前可见样式，不承担事实、权益或操作说明。
...
10. App 内不写“五星好评”，不先问用户是否喜欢，也不把反馈入口只留给不满意者。自动请求完全使用系统文案；手动入口只写当前动作。

### 3.1 Apple Watch 设置

- 设置入口与页面标题统一使用“Apple Watch”；状态只使用“正在检查 / 尚未配对 / 尚未安装 / 已安装 / 不可用”。
- 已安装时说明用户能完成的任务：“可在 Apple Watch 上签到并查看今天的状态。”未安装时只给真实下一步：“请在 iPhone 的 Watch App 中安装一日一印。”
- 尚未配对或尚未安装时，提供动作“打开 Watch App”，跳转到 iPhone 的 Watch App；不展开系统教程。
- 动效设置统一命名“波浪动态”；说明发生条件，不写省电、隐私或实现机制：“手表 App 在前台显示时，波浪缓慢流动。”

### 3.2 Widget 画廊文案边界

- 精确中英文正文只存在于 `settings.widget.style.<style>.detail`，本合同不复制第二份运行时文案。
- 描述不得把装饰解释成不存在的数据，也不能承担价格、权益、操作结果或错误说明。
- 中英文都应自然、准确。
...
以下词语属于工程或合同内部语言，不得直接暴露在用户可见的说明、状态、错误和购买文案中：

- 主承诺、高阶权益、小组件构图、小组件事实、共享存储、数据校验、虚构价格；
- widget facts、shared store、Advanced Benefits、Main Commitment。

## 5. 验收

- 每次修改用户文案都要同时检查简体中文与 English，并验证 String Catalog 可解析。
- Widget 画廊正式字符串须提供简体中文与英文。
