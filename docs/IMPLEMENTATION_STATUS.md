...
- Apple Watch Ultra 3 49mm / watchOS 26.4 Simulator 已对当前单核周环版完成待签、已确认与待同步三态整屏复核：系统时间与左上逻辑日期分离，中央主印、六日缺口周环、真实状态和 44 pt 重试动作均无裁切；完成印不再因 disabled Button 被系统压暗。WatchConnectivity 既有即时 response / Application Context 与无 `_dispatch_assert_queue_fail` 证据仍成立，但模拟器视觉不外推为真实配对 Watch、complication、Smart Stack 或真机验收。
...
- Apple Watch SE 3 40 mm / watchOS 26.4 Simulator 已对当前单核周环版完成待签、已确认与待同步三态整屏复核，无滚动指示、裁切或重叠。watchOS Simulator 不支持通过 `simctl ui content_size` 切换 Dynamic Type，因此最大字号仍保留为真机门禁，不能用代码分支存在代替视觉通过。
