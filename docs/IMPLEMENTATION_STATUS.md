...
- 设置支持链路已 clean-break 为正式“帮助与反馈”分组：原生反馈与建议页、帮助中心、隐私政策分别承担主动联系、自助排障和隐私说明；旧 `PulseExternalLinks`、旧“产品支持”入口与重复版本拼接已删除。`PulseSupportContract` 集中持有支持邮箱、URL、反馈/截图上限与版本元数据；反馈校验不截断，诊断快照逐项可见且覆盖日历/提醒/权益/数据规模等真实排障信息。用户可主动选择一张截图，系统 Picker 只暴露所选项，独立处理器去元数据、限输入/像素/输出后才附加。系统 Mail 的取消/草稿/排队/失败状态诚实区分；主承诺正文、签到时间/历史明细、记事正文、今日入镜媒体、备份、口令和设备 ID 不进入诊断。公开权威隐私/支持源码已同步，生产部署仍是独立门禁。
