...
- Home Screen Widget 关闭系统默认内容边距并实现四套独立小/中号比例：断层双色按用户后续明确选择将圆点节律固定左下，越界巨环只保留一个受控裁切的主环，承诺宣言以主承诺为第一读，错版撕页使用 18% 状态顶带与超大日期。四式都消费同一日/月、规范化主承诺、七日事实与单向 AppIntent，但按原型分别用文字、日印内核、色场和七日末点表达今日状态，不强塞通用顶栏或重复状态文案。所有 Widget 日印都由同一代码原语绘制，独立右下补形与 Accessory 位图副本已删除，保留干净的 312° 开口；断层双色通过独立方向参数把缺口固定到右侧状态色场，其他三式和 Accessory 显式保持原方向。七日圆点或方块由低强调线连接为单一时间轨迹，线条只连接节点外缘且不穿过空心内部。Accessory 使用“节律汇印”：圆形只呈现今日印记；矩形不显示日期，过去六日节点由真实连接线汇入右侧唯一今日印记；尺寸从系统边距后的可用容器推导，待办态整块绑定同一 AppIntent，完成态静态。Lock Screen、StandBy 与 Always-On 不渲染主承诺正文或 Home Screen 样式，任何 Widget 都不显示“为什么重要”。事实或样式变化成功后请求 timeline reload。
...
- Accessory “节律汇印”重构后重新执行 107 个单元/集成测试，全部通过；其中 7 个 Widget 快照测试独立定向通过，新增断言证明过去六日计数不会重复包含今天。Debug Simulator 构建、18 项品牌生成资产、String Catalog JSON 与 diff whitespace 检查同步通过。“四种样式均可选、重启后保持”的 UI 测试属于此前定向证据；原有 12 个 UI 测试的全量通过也属于本次 Accessory 视觉源码之前，不冒充本轮重新运行。
...
- Widget 自动化覆盖七日状态、过去六日完成计数、规范化主承诺名称投影、身份确认门、实际提交回执、DST/项目时区零点和空 store 只读；另覆盖四值偏好往返、默认 `faultField`、未知值失败关闭，以及样式变化只触发一次 timeline reload。锁屏不渲染名称、整块命中、完成态镂空勾、VoiceOver 单元素朗读与当前真机视觉仍需独立人工证据。
...
- 设计合同 GO：四式小号/中号方向、断层双色默认与左下圆点、Accessory “节律汇印”、代码原生日印、系统字体/语义色、默认隐私、不可用态和单向签到边界已冻结在 [Widget 共享 Store 合同](./WIDGET_SHARED_STORE_CONTRACT.md) 与 [品牌合同](../design/BRAND_SPEC.md)；合同冻结不等同于新 Accessory 实机界面通过。
...
1. 在真实 iPhone Widget Gallery 与已安装锁屏先复验新 Accessory 圆形/矩形的待办和完成态：开放环无伪勾、矩形无日期、六日轨迹汇入今日印记、右侧无裁切、整块可点且 VoiceOver 只朗读一个元素；同时补录设备型号与 OS 版本。
