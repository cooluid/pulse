...
二进制文件只存在 App 私有 `Pulse/Media/originals` 与 `Pulse/Media/thumbnails`，不作为 SwiftData Blob，也不放进 App Group。路径只能是两段相对路径，拒绝绝对路径、`..`、反斜杠、符号链接和未知扩展名。
...
Watch 只有收到 iPhone Repository 的创建或幂等回读回执后才能进入 `committed`。本地已入 outbox 但未收到回执为 `pendingSync`，不得宣称已签到或触发成功反馈；拒绝与持久化失败均显示失败并保留明确重试路径。Watch 快照是可清理、可重建投影；outbox 是收到匹配回执或用户明确重试/清除前必须保留的 durable 传输状态。二者都不是第二份签到历史。
...
2. 两个文件以随机 UUID 不可变路径安装到 App 私有正式目录，并在提交前完成 size/SHA-256 回读校验；该提交前校验可以对文件暂不可读或身份暂不一致进行短暂、有界重读，最终仍不一致则清理新文件并失败；
3. 原图和缩略图各自的 size/SHA-256 与元数据一起提交 Repository；
4. 重拍提交成功后才清理旧文件；数据库失败则删除新文件。

进程在步骤 2 后退出只会产生孤儿文件；启动审计根据正式 `ImprintMedia` 引用删除孤儿。任何缺失的被引用文件是完整性错误，不能显示“影像已保存”。打开已提交照片时仍按 size/SHA-256 校验；文件暂不可读可有界重读，身份不一致立即失败。
