...
- Lock Screen 圆形：今日开放环与实心完成印内部都显示由当前 `LogicalDay` 派生的本地化纯数字，不带“日”等日期后缀；待签到开放环不得出现会被误读为完成勾的斜线，完成态以实心内核替代勾；
- Lock Screen 矩形：左上显示今日短状态，过去六日节点上方显示由各自 `LogicalDay` 派生、不带日期后缀的本地化纯数字，并以真实连接线汇入右侧唯一今日印记；今天数字显示在大印内部，不显示主承诺文字、星期，也不把今天重复成第七个小节点；
...
- Lock Screen、StandBy 和 Always-On 只显示带当日日号的抽象印记与最小今日状态，不显示完整年月、星期或主承诺名称；
- Home Screen 固定显示用户已确认的主承诺名称，任何 Widget 都不显示“为什么重要”；名称与事实不写入 UserDefaults 或第二份投影真源，`widget.style` 只能选择四种 Home Screen 构图；
- Widget 使用非纯颜色开放环/实心完成印、节点形态、状态文字、系统字体和语义动态颜色；Accessory 的日号与矩形六个历史节点从辅助功能树隐藏，由整块标签统一朗读今日状态和过去六日已留印数量；日号必须由真实逻辑日和当前 Locale 派生，不持久化、不硬编码，也不替代开放环/实心内核与节点的非颜色状态；不使用 emoji、奖励或只靠颜色区分状态；
...
- Home Screen 从正式快照显示规范化主承诺名称且身份编辑后刷新；四值 `widget.style` 可持久化并在切换时刷新，未知值失败关闭；Lock Screen 不渲染名称，任何 Widget 都不显示说明或读取 Home Screen 构图。Accessory 矩形只投影过去六日统计和最近七个逻辑日的本地化日号；前六个日号对应历史节点，今日日号进入今日印记。日号是可重建展示值，历史计数不得把今天重复计入。
