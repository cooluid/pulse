...
- 构图 AppEnum、逐实例默认值、未授权时的明确拒绝，以及不同 Home Screen 实例可同时使用不同构图；枚举必须只覆盖待落之处 / 星环 / 叠印 / 数影 / 手札 / 静场 / 来路 / 潮痕，且不接受旧 `seal` 或旧五式标识；
