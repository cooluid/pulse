...
- Watch App 至少覆盖当前支持范围内的 40 mm 小屏和一台真实 Watch：标准字号的完成态/待签到态必须在一屏完整显示项目日期、今日日印、状态与七日脉冲且无滚动指示；无快照恢复态必须完整显示 44 pt 重试动作；Accessibility Dynamic Type 以及带七日脉冲的失败恢复态允许滚动但不得裁切、重叠或伪造日期。Watch App 主印是今日，页内七日脉冲只画六个历史节点；矩形 complication / Smart Stack 才使用“六历史 + 一今日”，且今日印记必须大于历史节点，不得因缩屏改成七个同权节点。完成印为实心鲜草，不得再叠半透明黑斑。
