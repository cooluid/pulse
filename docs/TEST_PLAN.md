...
- App 构图画廊与真实 Home Screen 小号/中号必须分别验证落印、叠印、数影、手札、静场、来路，并以 [widget-ritual-objects](./prototypes/widget-ritual-objects/) 为构图对照。落印检查大开口日环与完成闭合；叠印检查最多三层纸边且日号不与数影抢主角；数影检查 `1 / 10 / 11 / 31` 的半屏完整字形；手札检查蜡封与等大邮戳；静场检查残影从右下印心扩开且不是手札放大版；来路检查由远及近的六日印与今日环心日号。画廊必须使用同一正式渲染器和当前事实，末款完整可见。迁六式完成前，旧五式画廊不得当作本条通过。
- Home Screen 待签到时整块是唯一 AppIntent 命中面，完成态静态；六式、Accessory 与 Always-On 不使用连续动画。Reduce Motion 下结果必须等价。模拟器画廊截图只能作为候选证据，真实 Widget host 仍须单独验收。
