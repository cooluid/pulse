...
自动化测试只守可观察的功能结果和系统硬限制，不读取生产源码来限定实现写法，也不锁定构图、视觉令牌、颜色、几何、图层或截图像素。渲染附件只供人工查看。
...
| 资产 | 当前资产可解码、生成器输出完整且幂等、AppIcon 满足系统文件要求；不锁定颜色、题材或令牌清单 |

## 3. UI 自动化

- 首启确认承诺、签到落盘、重启保持、History 出现。
- 签到主操作单击只签到；0.45 秒长按只提交一次签到并在成功后请求相机；VoiceOver 自定义动作可达。具体控件图形不参与测试。
...
自动化不得断言构图、颜色、视觉令牌、字体、间距、圆角、图层、具体控件位置、View 层级、SwiftUI 写法、动画材料、像素差或截图哈希。截图与录屏只用于产品负责人查看当前实现，不形成未来设计门禁。

## 7. 既有系统能力回归

- 免费本地通知：授权、拒绝、外部撤权、实际到达、改时、签到后取消、跨日。
- iOS 26 scheduled Live Activity：验证 Lock Screen、Dynamic Island 支持形态、直接签到、锁定认证、完成与结束、完整/部分接受、容量竞争、撤权和本地通知接续；同日不重复。具体构图和颜色只做人工查看。
- 高级功能页预览不得响应签到或写入主题/业务事实；价格、权益和状态只读正式来源。预览长什么样不进入自动化合同。
