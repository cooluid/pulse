...
- App 构图画廊与真实 Home Screen 小号/中号必须分别验证待落之处、落印、叠印、数影、手札、静场、来路、潮痕，并以 [widget-ritual-objects](./prototypes/widget-ritual-objects/) 的正式清单为构图对照。待落之处检查小号单轴、中号左右分区、待办坑心只写“空着”与完成实心勾，且不得出现开放日环或渐变；落印检查大开口日环与完成闭合；叠印在无历史、不同历史数量和待办/完成状态下都保持固定四页物理纸叠，纸层不表达数据，日期、承诺、状态与今日印必须全部收在最上层纸面内并与纸边保持稳定内距，且不与数影抢主角；数影检查 `1 / 10 / 11 / 31` 的半屏完整字形；手札检查只有一张独立笺纸、左笺边、最多两条低对比书写纹、蜡封与不带连接线的等大邮戳，不得出现第二张纸或叠印式厚度；静场检查无需放大即可辨认的残影从右下印心扩开，回响核心比旧稿加深但整张卡仍轻柔，且不是手札放大版；来路检查由远及近的六枚鞋底与趾印、无连续图表曲线、今日环心日号，并逐项验证顶部 `六日 · N 印` 与过去六日完成事实一致且 N 只在 0...6；潮痕只检查底部潮面、主潮唇、一道潮迹、开放日环，以及完成时潮面上移与闭环变化，且不得出现天空、云、太阳、鱼或独立天空渐变。八式均不得使用伪数据，并需在深浅色、着色与 Clear 下保持文字和主物件边界。画廊必须同时显示同一正式渲染器的小号与中号，不得只用中号预览替代小号验收；权益徽章不得抢过预览主物件；末款完整可见，旧五式不得重新进入枚举、字符串或截图基线。
- Home Screen 待签到时整块是唯一 AppIntent 命中面，且按钮标签必须钉死到系统分配的完整可见区域；完成态整块静态且不可撤销。Widget 不使用 App 内那种连续环境动画或密集 Timeline 帧；只允许早间 / 日间 / 晚间三种低幅静态氛围 entry，以及权威签到事实 reload 后的一次有限物件变装（见 [WIDGET_MOTION_CONTRACT.md](./WIDGET_MOTION_CONTRACT.md)）。测试必须证明集中边界最多生成五条 entry、时段不改变业务事实、下一逻辑日重新投影；不得断言 iOS 会准点展示。画廊逐卡“预览变化”必须在 2 秒内使用同一 Renderer 串联三种时段与签到变化、可重复播放、播放中不叠加任务、离页取消，且预览前后正式签到事实不变；锁定构图的预览与购买入口不得嵌套。八式必须分别验证自己的主物件发生变化，不能只检查共享签到圈。Reduce Motion 下直接呈现相同终态。模拟器画廊录屏只能作为候选证据，真实 Widget host 仍须单独验收。
- 在系统“编辑小组件”中验证逐实例构图：默认构图必须是待落之处；同一主屏至少放置两个不同构图，分别修改后互不影响；权益未验证或撤销时包括落印在内的收费实例明确显示“构图尚未解锁”，不能静默换成待落之处；Lock Screen“节律汇印”不出现无关构图参数；App Group 中不存在 `widget.style`。
