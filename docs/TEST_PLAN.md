...
| AppModel | 操作互斥；签到先于拍照；相机/媒体失败不撤销签到；跨日/前台审计；重置日志；完整备份 |
| Settings | 今日入镜邀请、空间、主题、语言、提醒；重置后清理；损坏偏好失败关闭 |
...
- Settings 显示媒体空间、隐私说明、完整加密备份与照片数量恢复确认。
...
- Today 仍以签到为英雄动作；今日入镜是签到后的第二层邀请，不抢占未签到状态。
- 照片保持真实色彩，卡片属于草野脉冲系统，不出现第二套取景器/胶片品牌。
