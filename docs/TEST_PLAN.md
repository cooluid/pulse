...
- Watch App 至少覆盖当前支持范围内的 40 mm 小屏和一台真实 Watch：标准字号的完成态/待签到态必须在一屏完整显示今日日印与环绕的六个历史节点，且无滚动指示；界面不显示星期或自定义顶栏日期，只保留右上系统时钟，项目日数字偏轴落在左下背景并可部分出屏。主印内不放日期或“签到 / 已签到”文案。完成/待签由印的空心/实心形状表达，异常态才显示状态文字；无快照恢复态必须完整显示 44 pt 重试动作；Accessibility Dynamic Type 允许滚动但不得裁切、重叠或伪造日期。矩形 complication / Smart Stack 才使用横向“六历史 + 一今日”，且今日印记必须大于历史节点，不得因缩屏改成七个同权节点。完成印为实心鲜草，不得再叠半透明黑斑。
