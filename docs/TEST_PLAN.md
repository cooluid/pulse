...
- Home Screen 待签到时整块是唯一 AppIntent 命中面，且按钮标签必须钉死到系统分配的完整可见区域；完成态静态。八式、Accessory 与 Always-On 不使用连续动画。Reduce Motion 下结果必须等价。模拟器画廊截图只能作为候选证据，真实 Widget host 仍须单独验收。
