...
- App 构图画廊与真实 Home Screen 小号/中号分别验证八式产品/事实边界：待落之处免费默认；收费式未解锁时明确拒绝；共用 `PulseWidgetHomeRenderer`；未签到整块唯一签到入口；完成态不可撤销；快照事实诚实（叠印纸层不映射历史、来路计数即时派生、无 4/7 分数进 Widget）。外观以人工截图验收。
