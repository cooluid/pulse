...
- 照片保持真实色彩；已有照片时伴生入口显示真实缩略影，完整预览、重拍和删除进入独立 Sheet，不出现第二套取景器/胶片品牌。
- 悬浮底栏命中区只能占用自身真实高度，选中项不得通过宽度挤压改变相邻入口；独立高阶权益页必须在待购买、购买中、待批准、已解锁与不可用状态分别截图审阅。同组权益项必须等宽满行，末项与恢复购买完整位于购买条上方。
- App 构图画廊与真实 Home Screen 小号/中号必须分别验证落印、叠印、数影、手札、静场、来路、潮痕，并以 [widget-ritual-objects](./prototypes/widget-ritual-objects/) 的正式清单为构图对照。落印检查大开口日环与完成闭合；叠印检查最多三层纸边、文字与日号完整且不与数影抢主角；数影检查 `1 / 10 / 11 / 31` 的半屏完整字形；手札检查蜡封与等大邮戳；静场检查残影从右下印心扩开且不是手札放大版；来路检查由远及近的六日印与今日环心日号；潮痕检查天际、潮唇、一尾鱼、无大日号，以及完成时潮位与闭环变化。画廊必须同时显示同一正式渲染器的小号与中号，不得只用中号预览替代小号验收；末款完整可见，旧五式不得重新进入枚举、字符串或截图基线。
- Home Screen 待签到时整块是唯一 AppIntent 命中面，且按钮标签必须钉死到系统分配的完整可见区域；完成态静态。七式、Accessory 与 Always-On 不使用连续动画。Reduce Motion 下结果必须等价。模拟器画廊截图只能作为候选证据，真实 Widget host 仍须单独验收。
- 在系统“编辑小组件”中验证逐实例构图：同一主屏至少放置两个不同构图，分别修改后互不影响；权益未验证或撤销时收费实例明确显示“构图尚未解锁”，不能静默换成落印；Lock Screen“节律汇印”不出现无关构图参数；App Group 中不存在 `widget.style`。
- 有原图和签到的记录详情在标准 Dynamic Type 下必须显示横向三分动作坞；Accessibility Dynamic Type 下必须显示“导出横跨第一行、两个删除并列第二行”。分别验证只签到、只影像、二者并存，以及三个按钮各自来源锚定的系统流程与删除后保留关系。
