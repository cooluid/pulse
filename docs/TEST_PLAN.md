...
| Settings / Store | 今日入镜邀请、空间、主题、语言、提醒；独立 VIP 页；StoreKit 动态价格；类型化能力目录；独立 Widget 画廊；重置后清理；损坏偏好失败关闭 |
...
- Settings 显示媒体空间、完整加密备份与照片数量恢复确认；VIP 购买和 Widget 构图选择使用各自独立页面。
...
- 悬浮底栏命中区只能占用自身真实高度，选中项不得通过宽度挤压改变相邻入口；独立 VIP 页必须在待购买、购买中、待批准、已解锁与不可用状态分别截图审阅。
- Widget 画廊与真实 Home Screen 小号/中号必须分别验证呼吸环、草窗、涟漪径、晨露；不得出现粗黑分隔线、斜切色块、负字距裁切或方形节律残留。
