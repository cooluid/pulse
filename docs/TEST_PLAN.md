...
- Watch App 至少覆盖 40 mm 小屏和一台真实 Watch：标准字号的待签到、确认中/待同步、已确认、需要同步与失败状态必须在一屏完整显示蓝色状态场，且无滚动指示；左侧动作/事实不与底部波面重叠失读，右侧节点使用离散位置且不可拖动，只保留系统时钟。无快照恢复态必须完整显示 44 pt 重试动作；Accessibility Dynamic Type 允许滚动但不得裁切或重叠。iPhone 设置首页必须直接显示 Apple Watch 入口，独立页显示真实配对/安装状态和“波浪动态”开关；开关持久化并随新快照到达 Watch。关闭开关、Reduce Motion、后台、Always-On、低亮度、需要同步与失败均不得继续刷新波浪。矩形 complication / Smart Stack 仍使用横向“六历史 + 一今日”，且今日状态必须大于历史节点，不得因缩屏改成七个同权节点。
- Watch App 前台流体场需在真实 Watch 分别录制待签与已确认状态的完整 24 秒循环；Scene 失活、手腕放下、Always-On / 低亮度和 Reduce Motion 必须停止时间线并保持同一事实终态，`submitting`、`pendingSync`、`failed` 与 `needsSync` 必须为纯黑无循环。模拟器连续截图像素差异只证明代码在更新，不替代真表流畅度、温升与 14 天电量门禁。
