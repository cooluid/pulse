...
- App 构图画廊与真实 Home Screen 小号/中号必须分别验证待落之处、落印、叠印、数影、手札、静场、来路、潮痕，并以 [widget-ritual-objects](./prototypes/widget-ritual-objects/) 的正式清单为构图对照。待落之处检查小号单轴、中号左右分区、待办坑心只写“空着”与完成实心勾，且不得出现开放日环或渐变；落印检查不规则开放墨迹与完成收束，不得退回通用同心圈；叠印在无历史、不同历史数量和待办/完成状态下都保持固定四页物理纸叠，纸层不表达数据，日期、承诺、状态与今日印必须全部收在最上层纸面内并与纸边保持稳定内距，且不与数影抢主角；数影检查 `1 / 10 / 11 / 31` 的半屏完整字形；手札检查只有一张独立笺纸、左笺边、最多两条低对比书写纹、蜡封与不带连接线的等大邮戳，不得出现第二张纸或叠印式厚度；静场检查无需放大即可辨认的残影从右下印心扩开，回响核心比旧稿加深但整张卡仍轻柔，且不是手札放大版；来路检查由远及近的六枚鞋底轮廓、无脚趾/爪印和连续图表曲线，今日鞋底包含日号与确认纹，并逐项验证顶部 `六日 · N 印` 与过去六日完成事实一致且 N 只在 0...6；潮痕只检查底部潮面、主潮唇、一道潮迹与单侧岸尺刻度，以及完成时潮面上移和刻度由低位切换到高位，且不得出现天空、云、太阳、鱼、黑色块、温度计或十字语法。八式均不得使用伪数据，并需在深浅色、着色与 Clear 下保持文字和主物件边界。画廊必须同时显示同一正式渲染器的小号与中号，不得只用中号预览替代小号验收；只有锁定构图保留权益徽章；末款完整可见，旧五式不得重新进入枚举、字符串或截图基线。
