...
| Settings / Store | 今日入镜邀请、空间、主题、语言、提醒；独立高阶权益页；StoreKit 动态价格；类型化能力目录；独立 Widget 画廊；App Store 评价 URL、7 / 30 / 100 里程碑、尝试记录持久化与重置；损坏枚举、时间、布尔与评价记录严格失败关闭，用户明确重置设置后可恢复 |
...
- Settings “关于”显示“在 App Store 评价”；链接使用正式 Apple ID。自动评价请求不得出现在首次确认、前 6 次签到、失败、长按拍照、Widget / Watch 或 UI test，里程碑尝试后不得重复或从较低阈值连续补弹。开发构建验证系统界面；TestFlight 不出现不能判为实现失败。
...
- 界面主题权益验证：纸页手记是未购买、清洁安装与完整重置后的唯一默认；静野/晴昼/月汐/棱镜刻度在未购买时保留真实预览并显示“锁 + 高级功能”，点击进入唯一购买页且不得改变当前主题。购买或恢复后四者立即可选并跨重启保持；退款/撤销/未验证权益时统一回到纸页手记并明确提示。五套主题分别验证签到、长按拍照、最近七日、当前连续、记事、历史、照片与 Accessibility 能力等价。
