...
- App 构图画廊与真实 Home Screen 小号/中号必须分别验证呼吸环、数影、深景、静序、七日谱。呼吸环检查均衡信息与等高环；数影检查 `1 / 10 / 11 / 31` 的半屏完整字形；深景检查六个真实历史节点由远及近汇入唯一今日印记；静序检查无七日冗余；七日谱检查单行七个日期状态且无次数摘要。画廊必须使用同一正式渲染器和当前事实，末款完整可见。
- Home Screen 待签到时整块是唯一 AppIntent 命中面，完成态静态；五式、Accessory 与 Always-On 不使用连续动画。Reduce Motion 下结果必须等价。模拟器画廊截图只能作为候选证据，真实 Widget host 仍须单独验收。
- 在系统“编辑小组件”中验证逐实例构图：同一主屏至少放置两个不同构图，分别修改后互不影响；权益未验证或撤销时收费实例明确显示“构图尚未解锁”，不能静默换成呼吸环；Lock Screen“节律汇印”不出现无关构图参数；App Group 中不存在 `widget.style`。
