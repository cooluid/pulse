...
- Widget 画廊与真实 Home Screen 小号/中号必须分别验证呼吸环、晨露、斜光、潮汐、隅色、静序、强信号、七日谱；原有六式与既有截图逐项比较构图、颜色、字体层级和圆点节律，不能因新增样式被连带重写。强信号须验证柔色日期带、原字形与紧凑信息组；七日谱在两种尺寸都必须为单行七个日期圆点，不得换行或出现方格。画廊末款必须完整可见并可点击。
- 强信号与七日谱从待签到到已签到只在权威事实更新后播放一次不超过 2 秒的数据转场；原有六式不新增设计转场。Reduce Motion、已完成初始载入和 Accessory / Always-On 表面保持静态。模拟器录屏只能作为候选证据，真实 Widget host 仍须单独验收。
