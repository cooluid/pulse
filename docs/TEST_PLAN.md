...
- App 构图画廊与真实 Home Screen 小号/中号必须分别验证待落之处、落印、叠印、数影、手札、静场、来路、潮痕，并以 [widget-ritual-objects](./prototypes/widget-ritual-objects/) 的正式清单为构图对照。待落之处检查小号单轴、中号左右分区、待办坑心只写“空着”与完成实心勾，且不得出现开放日环或渐变；落印检查大开口日环与完成闭合；叠印在无历史、不同历史数量和待办/完成状态下都保持固定四页物理纸叠，纸层不表达数据，日期、承诺、状态与今日印必须全部收在最上层纸面内并与纸边保持稳定内距，且不与数影抢主角；数影检查 `1 / 10 / 11 / 31` 的半屏完整字形；手札检查只有一张独立笺纸、左笺边、最多两条低对比书写纹、蜡封与不带连接线的等大邮戳，不得出现第二张纸或叠印式厚度；静场检查无需放大即可辨认的残影从右下印心扩开，回响核心比旧稿加深但整张卡仍轻柔，且不是手札放大版；来路检查由远及近的六枚鞋底与趾印、无连续图表曲线、今日环心日号，并逐项验证顶部 `六日 · N 印` 与过去六日完成事实一致且 N 只在 0...6；潮痕检查天际、潮唇、一尾鱼、无大日号，以及完成时潮位与闭环变化。除潮痕外的环境填充不得使用渐变或伪数据，并需在深浅色、着色与 Clear 下保持文字和主物件边界。画廊必须同时显示同一正式渲染器的小号与中号，不得只用中号预览替代小号验收；权益徽章不得抢过预览主物件；末款完整可见，旧五式不得重新进入枚举、字符串或截图基线。
- Home Screen 待签到时整块是唯一 AppIntent 命中面，且按钮标签必须钉死到系统分配的完整可见区域；完成态整块静态且不可撤销。Widget 不使用 App 内那种连续环境动画；允许 Timeline phase 关键帧与 entry 切换过渡（见 [WIDGET_MOTION_CONTRACT.md](./WIDGET_MOTION_CONTRACT.md)）。潮痕在 phase 切换与签到 reload 时云位/潮位/单鱼姿态应可辨认。Reduce Motion 下只保留当前帧。模拟器画廊截图只能作为候选证据，真实 Widget host 仍须单独验收。
