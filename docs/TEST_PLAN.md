...
- Home Screen 待签到时整块是唯一 AppIntent 命中面，且按钮标签必须钉死到系统分配的完整可见区域；完成态整块静态且不可撤销。Widget 不使用 App 内那种连续环境动画或 Timeline 装饰关键帧；只允许权威签到事实 reload 后的一次有限物件变装（见 [WIDGET_MOTION_CONTRACT.md](./WIDGET_MOTION_CONTRACT.md)）。Reduce Motion 下直接呈现相同终态。模拟器画廊截图只能作为候选证据，真实 Widget host 仍须单独验收。
