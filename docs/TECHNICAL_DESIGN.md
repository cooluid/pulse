...
Watch App 页级几何只由 `PulseWatchLayoutMetrics` 解析实际容器、safe area、可见区段和集中设计令牌；禁止设备型号/表径分支和页面内散落固定尺寸。标准字号以今日印为中心、历史节点环绕的无滚动表盘构图；Accessibility Dynamic Type 或主印低于最小边长时进入明确的纵向滚动路径，避免用裁切冒充适配。
