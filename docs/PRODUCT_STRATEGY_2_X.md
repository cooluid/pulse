...
- 加密备份 v1 有认证容器、大小与数量上限、身份、时间顺序和来源校验，失败前不删除现有事实；错误口令、篡改、未知或不完整版本失败关闭。
...
| 智能留印窗口 | 用户主动开启后，在提醒时间通过 ActivityKit 远程启动短时灵动岛仪式，并与本地/微信通道仲裁 | 需要持续调度、push token、安全、送达与退出维护；不是灵动岛皮肤收费 | P1 Plus，先通过日印仪式原型 |
| 多通道提醒 | 在免费本地提醒之外提供经正式平台批准的外部提醒；微信只是候选适配器 | 服务端调度、送达、撤权和合规需要持续维护 | P2 Plus，先通过微信资格门禁 |
...
微信不是 iOS 本地通知的替代品。免费本地提醒继续作为默认、离线可用的基础能力；微信只能作为用户主动绑定、可随时解绑的附加通道。
...
- 至少 30 个 TestFlight 用户完成 14 天影像实验；
- 至少 30% 的实验用户主动启用，启用者 14 天拍摄中位数达到 8 天；
