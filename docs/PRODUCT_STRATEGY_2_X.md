...
把基础提醒、导出、读取历史、无障碍或隐私锁放到付费层，会直接削弱信任。2026-08-15 的受控付费范围是单独的永久买断高阶权益：解锁 iOS 26 scheduled Live Activity 和七种额外 Home Screen Widget 构图（星环 / 叠印 / 数影 / 手札 / 静场 / 来路 / 潮痕）；未购买用户仍保留本地通知、待落之处、App 内基础签到、签到闭环、历史、备份与数据控制。是否有足够付费价值必须用真实转化与退款证据审查，不能因为已经实现就视为产品验证通过。
...
| 智能签到窗口 | 在 1.0 本地买断提醒之上，提供跨设备、更长周期或个性策略的远程 ActivityKit 调度，并与微信通道仲裁 | 持续服务只可为新增的调度/策略收费，不能向买断用户重复售卖现有本地提醒 | P1 Plus，先通过远程原型 |
