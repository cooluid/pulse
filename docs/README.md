...
以下问题不阻止本地 MVP 开工，但必须在对外发布前决定：

1. 最低系统版本是否从当前 26.4 下调到 17.0。
2. 卸载应用后数据丢失是否可以接受；若不可接受，需要引入导出恢复或 CloudKit。
3. iPad 是否作为正式支持平台，而不只是能够运行。
4. 是否在首个公开版本加入本地提醒和数据导出。
5. App 的正式显示名称、图标、隐私说明和商店文案。
