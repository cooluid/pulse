# Pulse 代理须知

一日一印（英文名 Pulse）是已经公开发布的本地优先日签到产品。
这份文件是编码代理的工作入口。人读 [`docs/README.md`](./docs/README.md)；代理先读这里，再按任务打开对应合同。

默认这是给人用的成品，不是过审材料。用户是成年人。审核员、律师、测试不是界面的读者。

## 人格

信任来自行为，不来自否定清单。
做了什么就说什么。没做的事写进测试和实现，不要写进界面、邮件、权限弹窗或隐私政策主文。
“更完整”不是功能理由。任何新增说明、步骤、开关、空状态教导，必须能证明它降低了真实错误；否则删掉。

禁止这四种人格（GPT 默认很爱）：

1. **审核员**：反复声明不收集、不上报、不跟踪、不读相册、不做人脸。
2. **教导员**：先解释流程再让人动手；用占位符教用户怎么说话。
3. **防御工程师**：把去元数据、像素上限、后台不上报、字段排除清单投影到 UI。
4. **防泄密者**：用户还没行动，就警告“请勿填写记事 / 照片 / 备份”。

否认得越全，嫌疑越大。没发生的事，不要主动提。
真伤害要说：清除无法恢复、备份密码找不回、删除签到会连带记事。这是后果，不是避嫌。

界面只准出现：此刻的任务、正在发生的事实、不可逆后果、系统强制的权限用途、可选能力的一句事实。
按钮是动作，不是「检查并…」。成功提示说结果，不分割法律责任。

## 硬事实

不要发明第二份状态。涉及签到、日期、统计、删除、导入、备份、权益时，先对合同和实现。

- `CheckInRecord` 是签到唯一真源；`ImprintMedia` 是独立影像事实，二者不得互相伪造。
- 同一项目、同一逻辑日最多一条签到。成功以持久化成功为准，不以动画或内存状态为准。
- 中文产品名「一日一印」，英文「Pulse」。用户侧说「我的一件事」「签到」「高级功能」「今日照片」；「主承诺」「高阶权益」「入镜资产」是内部词。
- 运行时文案只改 String Catalog。公开隐私/支持正文只在 `/Users/fanr/Documents/work/coco-web`；本仓 `site/` 只做正式 URL 跳转。
- `design/brand-tokens.json` 只是当前颜色资产的工程入口，令牌可随设计自由增加、删除、改名或重做。外观只由当前实现和人工观感决定。
- `1.1 (9)` 已于 2026-08-30 公开发布；`PulseSchema 1.1.1`、backup container v2 / payload v3 与 Watch 协议 v2 / 本地状态 v2 已成为公开兼容基线。当前 `main` 是 Build 10 开发线，未发布外观仍可直接重画，但不得破坏已经公开的数据与协议迁移责任。

未收集的数据类型、未附带字段、不后台收集、不做人脸推断、EXIF 剥离、尺寸上限：用测试锁定，不要用脚注向用户证明很干净。

## 美术自由与测试边界

- 项目文档不定义构图、配色、材质、图形题材、风格、参考、反参考、页面映射或“设计意图”。需要参考时，只服务当次任务，不沉淀成后续门禁。
- 设计稿、当前 SwiftUI、截图和商店素材都是可替换实现，不是未来设计必须兼容的合同，也不需要文档逐项许可。
- 自动化只守领域事实、功能可达、无障碍结果、隐私、交易诚实和系统硬限制。禁止测试锁定 SwiftUI 写法、View 层级、固定几何、像素、颜色值、视觉令牌清单、构图关系、素材题材或动效风格。
- 截图和渲染附件只供人看，不做 golden image、像素差、构图哈希或审美 GO 门禁。视觉是否通过由人判断。

## 按任务读合同

不要把合同全文抄进界面或新文档。改哪一类，先打开哪一份：

| 任务 | 先读 |
| --- | --- |
| 签到 / 记事 / 照片 / 删除 | [`docs/DOMAIN_CONTRACT.md`](./docs/DOMAIN_CONTRACT.md) |
| 范围、收费、验收 | [`docs/PRODUCT_REQUIREMENTS.md`](./docs/PRODUCT_REQUIREMENTS.md)、[`docs/RELEASE_SCOPE_1_1.md`](./docs/RELEASE_SCOPE_1_1.md) |
| 用户可见文字 | [`design/CONTENT_DESIGN_SPEC.md`](./design/CONTENT_DESIGN_SPEC.md) |
| 外观工程 | [`design/BRAND_SPEC.md`](./design/BRAND_SPEC.md) |
| Widget / 灵动岛 / 提醒 | [`docs/PULSE_RITUAL_CONTRACT.md`](./docs/PULSE_RITUAL_CONTRACT.md) |
| 备份加密 | [`docs/DATA_ENCRYPTION_CONTRACT.md`](./docs/DATA_ENCRYPTION_CONTRACT.md) |
| 当前是否可发布 | [`docs/IMPLEMENTATION_STATUS.md`](./docs/IMPLEMENTATION_STATUS.md) |

## 做完先问

1. 用户此时要完成的唯一任务是什么？界面是否只服务这件事？
2. 删掉所有「我们不…」「请勿…」「发送前你会看到…」之后，任务是否仍然清楚？能删就删。
3. 这段话是写给正在用的人，还是写给以后可能来查的人？后者放测试或政策短句，不要铺在主路径。

对照：

- 坏：请勿填写记事或备份。除截图外我们没有附带设备 ID，也不会在后台收集。
- 好：说说遇到的问题。技术信息随这封邮件发送，可关闭。
- 坏：用于拍摄。照片留在本机，不会上传，不做人脸识别。
- 好：用于拍摄今日照片。
